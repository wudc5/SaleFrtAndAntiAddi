...
//import com.cwl.spark.ml.job.SaleForecastJob.{gp_url, hiveContext, log, props}
...
/**
  * Created by wdc on 2017/8/1.
  */
object SaleForecastTESTJob extends SparkBaseJob{
  // 期号和销售额字段格式
  ...
  def getSaleAmount(province: String, gameid: String): Double ={
    var saleamount = 0.0
    val sql_avggameamount = String.format("select avg(amount) as avggameamount from (select sum(drawsaleamount) as amount, saledrawnumber from drawsaleTable where provincename = '%s' and gameid = '%s' GROUP BY saledrawnumber) a", province, gameid)
    ...
    if(avggameamount == null){
      val sql_avgprovinceamount = String.format("select avg(amount) as avgcityamount from (select sum(drawsaleamount) as amount, gamename,saledrawnumber from drawsaleTable where provincename = '%s' group by saledrawnumber, gamename) a", province)
      log.info("sql_avgcityamount: "+sql_avgprovinceamount)
      val avgcityamount = hiveContext.sql(sql_avgprovinceamount).first().getAs[java.math.BigDecimal]("avgcityamount").toString.toDouble
  ...
  override def runJob: Unit ={
    ...
    val provincelist = hiveContext.sql("select distinct(provincename) as provincename from drawsaleTable").collectAsList()
    log.info("citylist: "+provincelist)
    for(i <- 0 until provincelist.size()){
      val province = provincelist.get(i).getAs[String]("provincename")
      if(province != null){
        ...
        for(gameid <- gameidlist){
          ...
          for(drawnum <- drawnumlist){
            ...
            val sql1 = String.format("select sum(drawsaleamount) as saleamount from drawsaleTable where provincename = '%s' and gameid = '%s' and saledrawnumber = '%s'", province, gameid, drawnum1)
            val sql2 = String.format("select sum(drawsaleamount) as saleamount from drawsaleTable where provincename = '%s' and gameid = '%s' and saledrawnumber = '%s'", province, gameid, drawnum2)
            val sql3 = String.format("select sum(drawsaleamount) as saleamount from drawsaleTable where provincename = '%s' and gameid = '%s' and saledrawnumber = '%s'", province, gameid, drawnum3)
            val sql4 = String.format("select sum(drawsaleamount) as saleamount from drawsaleTable where provincename = '%s' and gameid = '%s' and saledrawnumber = '%s'", province, gameid, drawnum4)
            val sql5 = String.format("select sum(drawsaleamount) as saleamount from drawsaleTable where provincename = '%s' and gameid = '%s' and saledrawnumber = '%s'", province, gameid, drawnum5)
            val sql6 = String.format("select sum(drawsaleamount) as saleamount from drawsaleTable where provincename = '%s' and gameid = '%s' and saledrawnumber = '%s'", province, gameid, drawnum6)
            val sql7 = String.format("select sum(drawsaleamount) as saleamount from drawsaleTable where provincename = '%s' and gameid = '%s' and saledrawnumber = '%s'", province, gameid, drawnum7)
            val sql8 = String.format("select sum(drawsaleamount) as saleamount from drawsaleTable where provincename = '%s' and gameid = '%s' and saledrawnumber = '%s'", province, gameid, drawnum8)
            val sql9 = String.format("select sum(drawsaleamount) as saleamount from drawsaleTable where provincename = '%s' and gameid = '%s' and saledrawnumber = '%s'", province, gameid, drawnum9)
            val sql10 = String.format("select sum(drawsaleamount) as saleamount from drawsaleTable where provincename = '%s' and gameid = '%s' and saledrawnumber = '%s'", province, gameid, drawnum10)
            log.info("sql1: "+sql1)
            log.info("sql2: "+sql2)
            log.info("sql3: "+sql3)
            log.info("sql4: "+sql4)
            ...
            // get true amount
            val sql11 = String.format("select sum(drawsaleamount) as saleamount from drawsaleTable where provincename = '%s' and gameid = '%s' and saledrawnumber = '%s'", province, gameid, drawnum.toString)
            ...
            // 保存结果
            val sql12 = String.format("select provinceid, provincename from drawsaleTable where provincename = '%s'", province)
...
//            val cityid = hiveContext.sql(sql12).first().getAs[String]("cityid")
            ...
            val uuid = getUUID()
            val statis_res = List(resultset_statis(uuid, "draw", drawnum, getCurrentTime().split(" ")(0), provincename, provinceid, "", "", gamename, gameid, forecast_amount, true_amount))
