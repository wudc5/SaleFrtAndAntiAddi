      ...
      if(cityid != null){
        ...
        for(gameid <- gameidlist){
          ...
          for(drawnum <- drawnumlist){
            ...
            val uuid = getUUID()
            val statis_res = List(resultset_statis(uuid, "draw", drawnum, getCurrentTime().split(" ")(0), provincename, provinceid, cityname, cityid, gamename, gameid, forecast_amount, true_amount))
